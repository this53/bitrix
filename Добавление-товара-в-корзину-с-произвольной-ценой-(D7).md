...
Данный подход, позволяет использовать расчёт всех остальных полей, в том числе и цепочек скидок, и некоторых полей по умолчанию, например `BASE_PRICE`, что позволит в админке видеть реальную стоимость товара, и стоимость по которой товар отпускается (то есть будут работать все функции обратного вызова) но поле `PRICE` всё так же остаётся под контролем разработчика:
```php
   $item->setFields([
      'QUANTITY' => $quantity,
      'CURRENCY' => \Bitrix\Currency\CurrencyManager::getBaseCurrency(),
